/*   Vacancy.c                Version 1.17     */
/*   Last Modification : 11/18/94 17:00:56        */
/*-----------------------------------------------------------------*/
...
double **rhs		/*the current right hand side*/)
{
	...
	for(i = 0; i < nn; i++) {
	    ...
	    a[vs][is][i] += cpl * znew[vs][i];
	    a[vs][vs][i] += cpl * znew[is][i];
...
*/
		...
		si = soltoimp[s];
                ...
                a[vs][s][i]  += cpl * cs->nco[i] * ktv * cs->nuevo[vs][i] *
                                (dact[s][s][i]/den);
                dtot1 += ktv * ( (cs->newa[s][i])*(1.0+kti*cs->nuevo[is][i]) )/
                         (den*den);
                dtot2 += ktv * ( (cs->newa[s][i])*(kti*cs->nuevo[vs][i]) )/
                         (den*den);
                cs->newa[vs][i] += ktv * cs->nuevo[vs][i] * 
                                   (cs->newa[s][i])/den;
...
/*		a[vs][s][i]  += cpl * cs->nco[i] * tmp * cs->nuevo[vs][i] * dact[s][s][i];
		dtot += tmp * cs->newa[s][i];
		cs->newa[vs][i] += tmp * cs->nuevo[vs][i] * cs->newa[s][i];
*/
	    }
	}
	...
	if ( cs->type == TR ) {
	    /*trapazoidal rule setup*/
            ...
            a[vs][vs][i] += cpl * cs->nco[i] * dtot1;
            a[vs][is][i] -= cpl * cs->nco[i] * dtot2;
...
/*	    a[vs][vs][i] += cpl * cs->nco[i] * dtot; */
	...
	if ( FALSE ) { /*(methdata.defmodel == FULLCPL) && (as != -1) ) {*/
		...
		/*the time terms*/
		rhs[vs][i] -= cpl * ( cs->nco[i] * (cs->nuevo[as][i] - cs->newa[as][i])
		...
		/*right hand side terms*/
		rhs[vs][i] -=   cs->nco[i] * cpl * (cs->nuevo[as][i] - cs->newa[as][i])
...
{ 
    ...
    /*get the damage values, if specified*/
    /* YASER 11/94                       */

    Fetch( globalalpha(V,mat), "glob.alpha" );
    Fetch( damalpha(V,mat), "local.alpha" );
    Fetch( damwidth(V,mat), "dam.width" );
    Fetch( damcenter(V,mat), "dam.center" );
    ...
    if ( (imp != -1) ) {
        Fetch( Kcpl0[which(V)][mat][imp][CN], "neu.0" );
        Fetch( Kcpl0[which(V)][mat][imp][CM], "neg.0" );
        Fetch( Kcpl0[which(V)][mat][imp][CDM], "dneg.0" );
        Fetch( Kcpl0[which(V)][mat][imp][CP], "pos.0" );
        Fetch( Kcpl0[which(V)][mat][imp][CDP], "dpos.0" );
        Fetch( KcplE[which(V)][mat][imp][CN], "neu.E" );
        Fetch( KcplE[which(V)][mat][imp][CM], "neg.E" );
        Fetch( KcplE[which(V)][mat][imp][CDM], "dneg.E" );
        Fetch( KcplE[which(V)][mat][imp][CP], "pos.E" );
        Fetch( KcplE[which(V)][mat][imp][CDP], "dpos.E" );
/*
...
/************************************************************************
 *                                                                      *
 *      Vsink( ) - This routine computes the recombination of           *
 *  interstitials with extended defects.                                *
 *                                                                      *
 *  Original:   YMH 11/94                                               *
 *          Based on R. Huang's code modified by YMH. See Interst.c     *
 *                                                                      *
 ************************************************************************/
void Vsink( 
float temp,
...
double **rhs)           /*the current right hand side*/
{
    ...
    for(i = 0; i < nn; i++) {
       ...
       mat = nd[i]->mater;
        ...
        cpl = globalalpha(V,mat) * area[i];

        /* activate global sink/source */

           /* 1. Modify LHS terms */
           a[vs][vs][i] += cpl;
           a[vs][ps][i] -= cpl * dequ[vs][i];

           /* 2. Modify RHS term */
           rhs[vs][i] -= cpl* (znew[vs][i] - equil[vs][i]);


        /* compute the local defect coupling terms */
        cpl = damalpha(V,mat) * area[i];

        /* activate sink/source if we're in the right location */
        if (fabs(t1-damcenter(V,mat)) < damwidth(V,mat)) {
