/*   active.c                Version 1.11     */
/*   Last Modification : 12/7/94 22:34:05        */
/*-----------------------------------------------------------------*/
...
double **dnet)
{
	...
	for(j = 0; j < nv; j++) {
	    ...
	    /*compute the terms needed for the substitutional portion*/
            ifr = Ichg[N][j] * Kcpl[I][mat][imp][N] + Ichg[M][j] *
                  Kcpl[I][mat][imp][M] + Ichg[D][j] * Kcpl[I][mat][imp][D] +
                  Ichg[R][j] * Kcpl[I][mat][imp][R];

            pifr = pIchg[N][j] * Kcpl[I][mat][imp][N] + pIchg[M][j] *
                   Kcpl[I][mat][imp][M] + pIchg[D][j] * Kcpl[I][mat][imp][D] +
                   pIchg[R][j] * Kcpl[I][mat][imp][R];

            vfr = Vchg[N][j] * Kcpl[V][mat][imp][N] + Vchg[M][j] *
                  Kcpl[V][mat][imp][M] + Vchg[D][j] * Kcpl[V][mat][imp][D] +
                  Vchg[R][j] * Kcpl[V][mat][imp][R];

            pvfr = pVchg[N][j] * Kcpl[V][mat][imp][N] + pVchg[M][j] *
                   Kcpl[V][mat][imp][M] + pVchg[D][j] * Kcpl[V][mat][imp][D] +
