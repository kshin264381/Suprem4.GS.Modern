/*   block_set.c                Version 1.8     */
/*   Last Modification : 9/1/94 14:37:07        */
/*-----------------------------------------------------------------*/
...
double *vacan)			/*scaled vacancy conc*/
{
    ...
    for(i = 0; i < nn; i++) {
        ...
        kti = Kcpl[I][nd[i]->mater][imp][CN];
        ktv = Kcpl[V][nd[i]->mater][imp][CN];
        val[i] = chem[ss][i] * inter[i]/
                 (1.0 + kti*inter[i]*equil[is][i] + ktv*vacan[i]*equil[vs][i]);
        lval[i] = log(val[i]);
        sqval[i] = sqrt(val[i]);
        dval[ss][i] = val[i] / chem[ss][i];
        dval[is][i] = val[i]*val[i]*(1.0+ktv*vacan[i]*equil[vs][i])/
                      (chem[ss][i]*inter[i]*inter[i]*equil[is][i]);
        dval[vs][i] = - ktv*val[i]*val[i] / (chem[ss][i]*inter[i]);
        dval[ps][i] = - val[i] * dequ[is][i] / equil[is][i];
/*
	val[i] = chem[ss][i] * inter[i];
	lval[i] = log(val[i]);
	sqval[i] = sqrt(val[i]);
...
*/
    ...
    for(i = 0; i < nn; i++) {
        ...
        sqval[i] = sqrt(val[i]);
        dval[ss][i] = val[i] / chem[ss][i];
        dval[is][i] = - kti*val[i]*val[i] / (chem[ss][i]*vacan[i]);
        dval[vs][i] = val[i]*val[i]*(1.0+kti*inter[i]*equil[is][i])/
                      (chem[ss][i]*vacan[i]*vacan[i]*equil[vs][i]);
        dval[ps][i] = - val[i] * dequ[vs][i] / equil[vs][i];
/*
	val[i] = chem[ss][i] * vacan[i];
	lval[i] = log(val[i]);
	sqval[i] = sqrt(val[i]);
