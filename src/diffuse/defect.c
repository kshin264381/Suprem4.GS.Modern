/*   defect.c                Version 1.13     */
/*   Last Modification : 11/18/94 17:01:20        */
/*-----------------------------------------------------------------*/
...
double *area)
{
	...
	Icoupling( temp, area, chem, equil, dequ, newb );
        /* calculate change in loop radius as well as interstitial sink*/
	Isink( temp, area, chem, equil, dequ, newb );
	val = inter;
    }
    else if ( imp == V ) {
        Vcoupling( temp, area, chem, equil, dequ, newb );
	Vsink( temp, area, chem, equil, dequ, newb );
	val = vacan;
    }
