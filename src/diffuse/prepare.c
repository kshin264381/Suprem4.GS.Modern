/*   prepare.c                Version 1.6     */
/*   Last Modification : 7/21/94 15:51:26        */
/*-----------------------------------------------------------------*/
...
#define Alloc(P,O,N) if(P) free((char *) P); P = scalloc( O, N);
...
void ia_calc( )
{
    ...
    if ( irow != NULL ) free( (char *) irow );
    if ( jrow != NULL ) free( (char *) jrow );
    ...
    if ( irow_d != NULL ) free( (char *)irow_d );
    if ( jrow_d != NULL ) free( (char *) jrow_d );
