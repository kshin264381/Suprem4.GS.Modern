/*   setup.c                Version 1.6     */
/*   Last Modification : 7/21/94 15:51:27        */
/*-----------------------------------------------------------------*/
...
double *rhs2)		/*the two norm of the right hand side*/
{
    ...
    get_active( FALSE, cs.temp, cs.nuevo, net, dnet, cs.newa, equil, dequ );

    /*compute the scaled defect concentrations*/
    /*compute the number of free interstitials and vacancies*/
    /*$dir no_recurrence*/
    for(i = 0; i < nn; i++) {
	inter[i] = cs.nuevo[is][i] / equil[is][i];
	vacan[i] = cs.nuevo[vs][i] / equil[vs][i];
    }
	...
	setup_2d(bs, cs.temp, cs.nsol, cs.sol, inter, vacan, cs.nuevo, cs.newa, 
...
struct call_str *cs)
{
	...
	for(i = 0; i < nn; i++) {
	    ...
	    /*the time terms*/
	    rhs[sol][i] -= cpl*(cs->nco[i]*cs->nuevo[sol][i] - cs->oco[i]*cs->old[sol][i]);
	...
	for( i = 0; i < nn; i++) {
	    ...
	    rhs[sol][i] -=   cs->nco[i] * t1 * cs->nuevo[sol][i] 
