/*   FEsolve.c                Version 1.7     */
/*   Last Modification : 8/29/94 10:41:59        */
/*-----------------------------------------------------------------*/
...
#define CLOCK(S,A) times(&Before); A; times(&After); print_time(S, (int *) &Before, (int *) &After);
#define TICK(S) times( &After); print_time(S, (int *) &Before, (int *) &After); times( &Before);
    ...
    int verbose)
{
    ...
    struct tms Before; struct tms After; times( &Before);
    FEverbose = verbose;
    ...
    free((char *)deln); free((char *)delo); free((char *)solo); free((char *)derr);
	...
	if (pv) { FEscramble( pv); free((char *)pv); }

    free((char *)ia);if(il!=ia)free((char *)il);free((char *)a);free((char *)l);free((char *)rhs);
}
    ...
    int *ia, int aoff, int *il, int loff)
{
    ...
    free((char *) arhs);
    free((char *) l);
    free((char *) work); free((char *) mega); free((char *) ipvt);
    ...
    free((char *)a); free((char *)work); free((char *)ipvt); free((char *) mega); free((char *) rhs);
}
