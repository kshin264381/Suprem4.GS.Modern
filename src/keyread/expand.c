...
int check_list(char* s, struct par_str** pars, int* best, struct par_str** match, int recurs);
...
//
{
	...
	best = compare( bexp->value.sval, cards[ cardnum ]->name);
...
//
{
    while ( param[0]->name[0] != '\0' ) {
	/*process this parameter*/
	param[0]->value.sval = (char *)malloc( sizeof(char)*20 );
