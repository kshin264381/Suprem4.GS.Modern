...
int *loff)	/*the offset*/
{
	...
	/*the workspace array*/
	if (wrkspc[si] != NULL) {free((char *) wrkspc[si] ); wrkspc[si] = NULL; }
	...
	if ( methdata.block == ICCG ) {
	    ...
	    for(j = 0; j < MAXBACK; j++) {
		if (p[j][si] != NULL) {free((char *) p[j][si] ); p[j][si] = NULL;}
		if (ap[j][si] != NULL) {free((char *) ap[j][si] ); ap[j][si] = NULL;}
		p[j][si]   = (double *)calloc( nn, sizeof( double ) );
		ap[j][si]  = (double *)calloc( nn, sizeof( double ) );
		cnt += 2;
	    }

	    if (r[si] != NULL) {free((char *) r[si] ); r[si] = NULL;}
	    if (qinvr[si] != NULL) {free((char *) qinvr[si] ); qinvr[si] = NULL;}
	    if (aqinvr[si] != NULL) {free((char *) aqinvr[si] ); aqinvr[si] = NULL;}
	...
	*l = (double *)malloc( (il[0][nsol*nn] + *loff + 1) * sizeof(double) );
	...
	print_time("symbolic factorization", (int *) &before, (int *) &after);
    }
