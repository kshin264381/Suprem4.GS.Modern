 ...
 * Modified: RYSH Aug94 (added loop radius and density to structure file)
 *---------------------------------------------------------------------*/
#include "global.h"
#include "constant.h"
#include "dbaccess.h"
...
/*            if ( sscanf( iline, "M %d %le", &sub_ornt, &last_temp) < 1 ) */
            /* Add loop radius, density, and L-factor */
            if ( sscanf( iline, "M %d %le %le %le %le", &sub_ornt, &last_temp,
                 &loopr, &rho, &looplength) < 1 )
...
/*    fprintf( lu, "M %d %e\n", sub_ornt, last_temp); */
    /* Add loop radius, density, and L-factor */
    fprintf( lu, "M %d %e %e %e %e\n", sub_ornt, last_temp, loopr, rho,
             looplength);
