...
struct par_str* param)
{
    ...
    if ( infile )
      {
       if (is_specified(param, "forest") && get_bool(param, "forest"))
	 {
	  (void) forest(infile, 0, SIMULATOR_SUPREM, 1);
	  S7Options.gridder = FOREST_GRIDDER;
	  return;
	 }
      }
	...
	if ((ier = squares( ivratio)) < 0) return;
	by_nd = TRUE;
    }
    create_db( by_nd );
