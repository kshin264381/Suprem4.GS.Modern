     ...
     int show)			/* Whether to plot the electrode setup. */
{
    ...
    free((char *)Pr1); free((char *)Ptconc); free((char *)Pnbc); free((char *)Pietype); free((char *)Ppt);
    return(0);
}
...
{
    int i, k;
    float *Pr1, *Ptconc, *tmp;
    char SelectBuf[100];
    
    Pr1 = scalloc( float, np);
    Ptconc = scalloc( float, np);
    tmp = scalloc( float, nn);
