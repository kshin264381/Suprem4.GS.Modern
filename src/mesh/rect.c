...
int read_bound (
    struct par_str* param,		/* Magic cookie for the parser */
...
void rect_boot()
{
    ...
    for (i = 0; i < MAXDIM; i++)
	for (j = 0; j < maxuser; j++) {
	    if (before && utag[ i][ j]) free((char *) utag[ i][ j]);
	    utag[ i][ j] = 0;
	}
    before++;
 ...
 *----------------------------------------------------------------------*/
void dvpram (double hl, double hr, double el, int *nnew, 
    ...
    struct par_str* param)
{
    ...
    /* Get its bounds */
    if (read_bound( param, &ureg[ nur-1][ 0], 1)) return;
    ...
    struct par_str* param)
{
    ...
    if (read_bound( param, &uedge[ nuj-1][ 0], 0)) return;
