...
int get_value(par_str **par, char *name, val_str *value)
{
    ...
    for( ; par[0] != NULL; par++ ) {
        ...
        if (par[0]->param != NULL) {
            i = get_value(par[0]->param, name, value);
            if ( i >= 0)
                return( i );
        }
    }
...
char *name)
{
    ...
    //if (get_value(((par_str *)par)->param, name, &value) != -1)
 ...
 *									*
 ************************************************************************/
char *get_string(par_str* par, char *name)
{
    val_str value;
 ...
 *									*
 ************************************************************************/
int is_specified(par_str* par, char *name)
{
    int temp;
    val_str value;

    /* Looks like par (int) to par_str conversion is causing problem */
//    if ((temp=get_value(((par_str *)par)->param, name, &value)) != -1)
//	return( temp );
