...
int read_list(struct par_str **par, FILE *fd)
{
	...
	if ( (eof = fread((char *) &temp, sizeof(int),1,fd)) != 1) {
...
int read_param(struct par_str *par, FILE *fd)
{
	...
	return( eof );
    if ( (eof = fread((char *)&par->type, sizeof(int), 1, fd)) != 1)
	return(eof);

    /*read in the default based on type data*/
    switch( par->type & ~ MASK ) {
    case REAL :	if ( (eof = fread((char *) &par->def.dval,sizeof(float),1,fd) != 1))
		    return( eof );
		break;
    case COMM :
    case BOOL :
    case CHOICE :
    case INT  :	if ( (eof = fread((char *)&par->def.ival, sizeof(int),1, fd) != 1))
		    return( eof );
		break;
...
FILE *fd)
{
    ...
    case OPER   : /*write out the four byte descriptor*/
		  if ((eof = fread((char *) &(bexp[0]->value.ival), sizeof(int), 1, fd))
								    != 1)
		      return(eof);
		  break;
    case REAL   : /*write out the real number*/
		  if ((eof = fread((char *) &(bexp[0]->value.dval),sizeof(float), 1,fd))
								    != 1)
		      return( eof );
		  break;
