/*   elast.c                Version 1.4     */
/*   Last Modification : 7/21/94 22:12:53        */
/*-----------------------------------------------------------------*/
    ...
    double dt)
{
    ...
    times(&after); print_time("Compressible matrix symbolism", (int *) &before, (int *) &after); times(&before);
    ...
    times(&after); print_time("Compressible matrix assembly", (int *)&before, (int *) &after); times(&before);

    /* Go solve the problem */
    numfac( 2*np, (int*)0, 1, ia, aoff, a, il, loff, l);
    numbac( 2*np, il, loff, l, rhs);

    times(&after); print_time("Compressible matrix solution", (int *) &before, (int *) &after); times(&before);
    ...
    free((char *)il); free((char *)ia); free((char *)rhs); free((char *)l); free((char *)a); free((char *)fix);
