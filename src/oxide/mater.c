/*   mater.c                Version 1.6     */
/*   Last Modification : 12/6/94 22:17:56        */
/*-----------------------------------------------------------------*/
...
struct par_str* param)
{
    ...
    fetch("density", Density(mat), 1.0, 0.0);
    ...
    if ( is_specified( param, "lcte") ) {
	 ...
	 if (atherm[ mat] != 0) free((char *) atherm[ mat]);
	...
	if (act_a[mat][type] != NULL) free((char *)act_a[mat][type]);
	...
	if (act_b[mat][type] != NULL) free((char *)act_b[mat][type]);
...
int ChosenBC(
     char *par, struct par_str* param, int other)
{
    int i, offset = other? 0:1;
    for (i = 0; i <3; i++)
