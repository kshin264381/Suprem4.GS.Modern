/*   do_1d.c                Version 1.5     */
/*   Last Modification : 7/24/94 12:36:39        */
/*-----------------------------------------------------------------*/
...
b_typ *
AddItfEdge(
    int ie, int j, int mat1, int mat2,
    b_typ **touched);

int d_compar_local(const void *ff1, const void *ff2);
...
int byarc)			/* whether to go by arclength or x */
{
	...
	for( t = 0; t < ne; t++ ) {
        ...
        /*sort the data*/ 
        qsort(data, count, sizeof(struct d_str), d_compar_local);
...
int d_compar_local(const void *ff1, const void *ff2)
{
 struct d_str *f1 = (struct d_str *) ff1;
 struct d_str *f2 = (struct d_str *) ff2;
	   ...
	   b_typ **touched)
{
	...
	/* Otherwise create it */
	nuevo = salloc( b_typ, 1);
	nuevo->ie = ie;
	nuevo->j  = j;
	touched[ 3*ie+j] = nuevo;

	/* Look clockwise, then anticlockwise */
	trotate( ie, (j+2)%3, 1, &je, &oje, &k, &kk);
	nuevo->left = AddItfEdge( oje, 3-k-kk, mat1, mat2, touched);
	trotate( ie, (j+1)%3, 0, &je, &oje, &k, &kk);
	nuevo->right = AddItfEdge( oje, 3-k-kk, mat1, mat2, touched);
