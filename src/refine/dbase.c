/*   dbase.c                Version 1.5     */
/*   Last Modification : 7/21/94 23:45:14        */
/*-----------------------------------------------------------------*/
...
{
   struct LLedge *nuevo,*first,*last;
  ...
  /*...Create nuevo link */
   ...
   /*...Include in linked list. */
    if (sreg[ir]->bnd == 0) {     	/* Initialize a region. */
       sreg[ir]->bnd = nuevo->next = nuevo->prev = nuevo;
       } 
    else {				/* Insert into existing region */
	first = (pos==BEFORE)? lep : lep->next;
	last =  first->prev;

	last->next = nuevo;	nuevo->prev  = last;
	nuevo->next  = first;	first->prev = nuevo;
       }
    sreg[ir]->len++;
