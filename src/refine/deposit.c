...
//extern void ReadInNewGeometry(char *filename);
...
/*       ReadInNewGeometry(sfile); */
    ...
    /* 
     *  If we are using Forest, let it do the deposit and regrid
     */
    if (S7Options.gridder == FOREST_GRIDDER)
      {
       ForestDeposit(mater, thick / 1e-4, space / 1e-4, imp, conc);
       return;
      }

    /* 
     *  If we are using SUPREM-IV's gridder, then we have to go
     *  through the tri gridder.
     */
    GridSave();
