    ...
    if (CHOSEN ("speedie"))
      {
       ...
       f1 = fopen("speedie/beam.mo","r");
       if (f1 == NULL)
	 {
	  printf("error opening beam.mo\n");
	  exit(1);
	 }
       
       f2 = fopen("speedie/diffusion.pi","r");
       if (f2 == NULL)
	 {
	  printf("error opening diffusion.pi\n");
	  exit(1);
	 }
       
       f3 = fopen("speedie/test.po","r");
       if (f3 == NULL)
	 {
	  printf("error opening test.po\n");
	  exit(1);
	 }
    ...
    if (S7Options.gridder == FOREST_GRIDDER)
      {
       ...
       ForestGetGeometryLimits(&xmin, &ymin, &xmax, &ymax);
    ...
    if (S7Options.gridder == FOREST_GRIDDER)
      {
       ...
       //double *ycoord = new double[nump];
    	xcoord = (double*)malloc(sizeof(double)*nump);
    	ycoord = (double*)malloc(sizeof(double)*nump);
       ...
       for (i = 0; i < nump; i++)
	 {
	  ...
	  ycoord[i] = p[1][i] * 1e4;
       ...
       ForestEtchCoord(mater, xcoord, ycoord, nump);
