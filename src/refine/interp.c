...
int r1, int r2)
{
    ...
    */
    ...
    free((char *)done);
    times(&after);
    print_time("total interpolation time", (int *) &before, (int *) &after);
}
...
int in, int ie2)
{
	...
	case iG  : case iGa  :
	    ...
	    nuevo = r * ans[1] + ir * ans[0];
	    set_sol_nd(in, i, exp(nuevo));
	    break;
	default:
	    ans[0] = sol_nd(vert_tri(ie2,0),i);
	    ans[1] = sol_nd(vert_tri(ie2,1),i);
	    nuevo = r * ans[1] + ir * ans[0];
	    set_sol_nd(in, i, nuevo);
	    break;
	}
    }
}
...
int in, int ie2)
{
	...
	case iG  : case iGa  :
	    ...
	    sol3x3(b, pvt, ans);
	    nuevo = ans[0] * cordinate(pt_nd(in),0) +
