...
int back)	/*flag for backgorund execution*/
{
	...
	*stdout = *t1;
	...
	if (fork() == 0) {
		...
		supbln = len;
		supbuf = sralloc(char, len * sizeof(char), supbuf );
	    }
...
int back)	/*the flag for background of the command*/
{
	...
	if (file != NULL)
	    if (param != NULL)
		str = (char *) malloc( strlen(file)+strlen(name)+strlen(param)+20 );
	    else
		str = (char *) malloc( strlen(file)+strlen(name)+20 );
	else
	    if (param != NULL)
		str = (char *) malloc( strlen(name)+strlen(param)+20 );
	    else
		str = (char *) malloc( strlen(name)+20 );
...
int no_exec)
{
    ...
    if (check(par, cards[command[index].cmd_num]) == -1) {
    ...
    if (! no_exec) {
	command[index].func( par, cards[command[index].cmd_num] );
    }
    return;
}
