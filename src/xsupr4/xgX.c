 ...
 */
{
    GraphWidget gw = (GraphWidget) new_stuff;
    ...
    wi->dev_info.axis_height = \
    		gw->graph.labelFont->max_bounds.ascent + \
 ...
 */
{
    ...
    XDrawString(XtDisplay(gw), XtWindow(gw), loc_gc,
 ...
 */
{
    ...
    if (style == L_AXIS) {
        ...
        if (lappr == 0) {
            ...
            segGC(w, gw->graph.pix[color%MAXCOL], LineOnOffDash, width, (char*)dashlist[lappr], 2);
        }
    }
...
/* dummy function that stops drawing sequence */
